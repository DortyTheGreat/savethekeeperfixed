...
GameEvents.Subscribe("custom_private_message", function(data) {
    $.Msg("Private: " + data.text);
    GameUI.SendCustomChat(data.text); // or show in your custom panel
	GameEvents.SendEventClientSide("dota_chat_add_message", {
            player_id: -1,
            team_only: false,
            message: data.text
        });
...
(function () {
    GameEvents.Subscribe("custom_message_event", function(data) {
        $.Msg("Custom message: " + data.msg);
		//GameEvents.SendEventClientSide("dota_hud_chat_message", {
        //    player_id: -1,
        //    team_only: false,
        //    message: data.text
        //});
        // You can also show it in UI, HUD panel, chat, etc.
		//$.DispatchEvent("DOTAShowTextTooltip", $("#CustomChatLine"), "your message");
		var chat = $("#CustomChatLine");
        if (chat) {
            chat.text = data.msg;
        }
    });
