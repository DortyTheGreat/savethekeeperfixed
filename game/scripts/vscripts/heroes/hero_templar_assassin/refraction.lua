...
function ApplyModifiers(keys)
    ...
    if caster:HasAbility("special_bonus_unique_templar_assassin") then
		local talent = caster:FindAbilityByName("special_bonus_unique_templar_assassin")
        stacks = stacks + talent:GetSpecialValueFor("value")
    end
	if caster:HasModifier("modifier_damage_absorb") then	
...
function RemoveDamageAbsorbStack(keys)
	...
	local heal_threshold = ability:GetLevelSpecialValueFor( "heal_threshold", ability:GetLevel() - 1 )
	...
	if caster:HasModifier(modifier) then
		...
		if damage >= damage_threshold then
			...
			local healamount = damage * heal_threshold / 100
			caster:SetHealth(caster:GetHealth() + damage + healamount)
			SendOverheadEventMessage( caster, OVERHEAD_ALERT_HEAL , caster, healamount, nil )
