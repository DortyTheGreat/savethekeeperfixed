...
function GameMode:OnHeroInGame(hero)
	if hero:IsRealHero() then
		...
		local pos_vector = Entities:FindByName( nil, "ve_keeper_6" ):GetAbsOrigin()	
		CreateUnitByName( "npc_dota_ve_keeper", pos_vector, true, nil, nil, 6)
	end
end

function PickCreepsUnderThreshold(threshold)
    local creeps = {}
    for name, data in pairs(GameRules.unitdata) do
        if data.cost and data.cost > 0 then
            table.insert(creeps, {name = name, cost = data.cost})
        end
    end

    -- Randomize order to make selection varied
    for i = #creeps, 2, -1 do
        local j = RandomInt(1, i)
        creeps[i], creeps[j] = creeps[j], creeps[i]
    end

    local selected = {}
    local total_cost = 0

    -- Try to add as many creeps as possible without exceeding threshold
    for _, creep in ipairs(creeps) do
        if total_cost + creep.cost <= threshold then
            table.insert(selected, creep.name)
            total_cost = total_cost + creep.cost
        end
    end
...
function GameMode:OnGameInProgress()
	...
	Timers:CreateTimer(0.1, function()
		...
		--GameMode:spawnCreep("basic_unit_melee_1", 1, Entities:FindByName( nil, "spawn_" .. 6):GetAbsOrigin(), Entities:FindByName( nil, "point_" .. 6), 6, nil)
		--GameMode:spawnCreep("basic_unit_melee_1", 1, Entities:FindByName( nil, "spawn_" .. 6):GetAbsOrigin(), Entities:FindByName( nil, "point_" .. 6), 6, nil)
