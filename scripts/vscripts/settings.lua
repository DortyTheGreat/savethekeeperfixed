...
-- Внутриигровые динамические переменные, используемые и изменяемые скриптом.
GAME_STATE_NUMBER = 0 			-- номер текущей стадии.
CURRENT_STATE_TIME = 0			-- текущее время стадии (время до увеличения ограничения еды).
ROUND_TIME = 0.0 				-- текущее время до следующего раунда (волны).
INCOME_TIME = 0.0				-- текущее время до получения дохода.

-- Внутриигровые константы, используются и не изменяются скриптом.
...
STATECHANGE_TIME_CONSTANT = 200	-- через сколько секунд произойдёт смена стадии.
CREEP_ARENA_BUFF = false		-- давать крипам пассивную способность арены? true - давать; false - не давать.
CREEP_GOLD_REWARD_CONST = 1;	-- коэффициент награды за убийство вражеского крипа (сколько будут давать золота за убийство крипа)
UPGRADE_COST_CONST = 50;		-- коэффициент стоимости покупки улучшения (умножается на текущий уровень улучшения и прибавляется к текущей стоимости)
CREEP_SPAWN_DELAY = 0.5;		-- периодичность спавна крипов
...
if GetMapName() == "8_paladins" then
	...
	DISABLE_FOG_OF_WAR_ENTIRELY = true     -- Should we disable fog of war entirely for both teams?
elseif GetMapName() == "4_paladins" then 
...
elseif GetMapName() == "paladins_arena" then 
	...
	ROUND_DELAY = 45.0
	INCOME_COUNT = 6
	CREEP_GOLD_REWARD_CONST = 0.3
	CREEP_ARENA_BUFF = true
