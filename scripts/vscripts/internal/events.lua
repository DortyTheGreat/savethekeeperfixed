...
function GameMode:_OnGameRulesStateChange(keys)
  ...
  elseif newState == DOTA_GAMERULES_STATE_HERO_SELECTION then
    ...
    local status, err = pcall(function() GameMode:OnAllPlayersLoaded() end)
	if not status then
  ...
  elseif newState == DOTA_GAMERULES_STATE_GAME_IN_PROGRESS then
    ...
    local status, err = pcall(function() GameMode:OnGameInProgress() end)
	if not status then
		CustomGameEventManager:Send_ServerToAllClients( "error_debuger", {g_error = err, status = status} )
		print("\n *=* [VE ERROR] ********** " .. err)	
	end	
...
function GameMode:_OnNPCSpawned(keys)
  ...
  if npc:IsRealHero() and npc.bFirstSpawned == nil then
    ...
    local status, err = pcall(function() GameMode:OnHeroInGame(npc) end)
	if not status then
		CustomGameEventManager:Send_ServerToAllClients( "error_debuger", {g_error = err, status = status} )
		print("\n *=* [VE ERROR] ********** " .. err)		
	end	
