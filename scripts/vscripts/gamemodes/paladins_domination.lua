...
function GameMode:OnGameRulesStateChange(keys)
  ...
  if newState == DOTA_GAMERULES_STATE_PRE_GAME then		
	local pos_vector = Entities:FindByName( nil, "spawn_tower"):GetAbsOrigin()	
	local crunit = CreateUnitByName( "npc_dota_tower_domination", pos_vector, true, nil, nil, DOTA_TEAM_NEUTRALS)
	...
	for playerID = 0, DOTA_MAX_TEAM_PLAYERS-1 do
		...
		if PlayerResource:GetConnectionState(playerID) == DOTA_CONNECTION_STATE_CONNECTED then
			--print(PlayerResource:GetSelectedHeroID(playerID))
			if PlayerResource:GetSelectedHeroID(playerID) ~= -1 then
				AddFOWViewer(PlayerResource:GetTeam(playerID),crunit:GetAbsOrigin(), 999999.0, 999999.0, false)
			end
		end
	end
	print("CHANGES READY")
